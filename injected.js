...
(function () {
  ...
  style.innerHTML = `
    @keyframes sg-slide-in { 0% { transform: translateY(-30px) scale(0.95); opacity: 0; } 100% { transform: translateY(0) scale(1); opacity: 1; } }
    @keyframes sg-pulse-glow { 0% { box-shadow: 0 0 10px rgba(248, 81, 73, 0.2); } 50% { box-shadow: 0 0 25px rgba(248, 81, 73, 0.5); } 100% { box-shadow: 0 0 10px rgba(248, 81, 73, 0.2); } }
    .sg-modal-container { position: fixed; top: 24px; right: 24px; z-index: 2147483647; width: 380px; background: linear-gradient(145deg, #0d1117, #161b22); color: #c9d1d9; border: 1px solid #f85149; border-radius: 12px; padding: 20px; font-family: 'Courier New', Courier, monospace; animation: sg-slide-in 0.4s cubic-bezier(0.16, 1, 0.3, 1) forwards, sg-pulse-glow 2s infinite; overflow: hidden; }
    .sg-btn { padding: 10px 18px; border-radius: 6px; cursor: pointer; font-weight: bold; transition: all 0.2s ease; font-family: inherit; font-size: 13px; outline: none; }
    ...
    return new Promise((resolve) => {
      ...
      div.innerHTML = `
        ...
        <div style="background: rgba(248, 81, 73, 0.1); border-left: 4px solid #f85149; padding: 12px; font-size: 13px; color: #f0f6fc; margin-bottom: 16px; line-height: 1.5; border-radius: 0 4px 4px 0;">
          <strong style="color: #ff7b72; font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px;">Security Alert</strong><br/>${reason}
        </div>
      ...
      document.getElementById('sg-rej').onclick = () => {
        ...
        setTimeout(() => { 
          div.remove(); 
          window.postMessage({ type: "SIGGUARD_BLOCKED" }, "*"); 
          resolve(false); 
        }, 200);
      };
    });
    ...
    Object.defineProperty(provider, 'request', {
      value: async function (args) {
        if (args.method === 'eth_sign' || args.method === 'personal_sign') {
          const reasonText = "You are about to sign a raw, unreadable message. Hackers use this to hide malicious transactions. Only proceed if you 100% trust this site.";
        ...
        if (args.method === 'eth_sendTransaction') {
          ...
          if (payload.startsWith('0x095ea7b3') && payload.includes('ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff')) {
