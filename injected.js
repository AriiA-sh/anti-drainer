console.log("🛡️ SigGuard: Advanced Cyberpunk UI Hooked!");

(function () {
  // تزریق استایل‌ها و انیمیشن‌ها به صورت داینامیک
  ...
  style.innerHTML = `
    ...
    .sg-modal-container::before {
      ...
      background: linear-gradient(to bottom, rgba(255,255,255,0), rgba(248,81,73,0.05), rgba(255,255,255,0));
      animation: sg-scanline 3s linear infinite; pointer-events: none;
    }
    .sg-btn {
  ...
  function askUser(title, data, reason) {
    return new Promise((resolve) => {
      ...
      div.innerHTML = `
        ...
        <div style="background: rgba(248, 81, 73, 0.1); border-left: 4px solid #f85149; padding: 12px; font-size: 13px; color: #f0f6fc; margin-bottom: 16px; line-height: 1.5; border-radius: 0 4px 4px 0; position: relative;">
        ...
        <div style="font-size: 11px; color: #8b949e; margin-bottom: 20px; background: #010409; padding: 10px; border-radius: 6px; border: 1px solid #30363d; font-family: monospace;">
          <div style="color: #58a6ff; margin-bottom: 4px;">// Intercepted Payload</div>
        ...
        <div style="display: flex; gap: 12px; justify-content: flex-end; position: relative; z-index: 10;">
          <button id="sg-rej" class="sg-btn sg-btn-abort">Abort Request</button>
          <button id="sg-acc" class="sg-btn sg-btn-proceed">I Understand, Proceed</button>
        </div>
      `;
      document.body.appendChild(div);
      
      // انیمیشن خروج نرم هنگام کلیک
    ...
    Object.defineProperty(provider, 'request', {
      value: async function (args) {
        ...
        if (args.method === 'eth_sendTransaction') {
          ...
          if (tx.data && tx.data.startsWith('0x095ea7b3') && tx.data.includes('ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff')) {
            const reasonText = "This contract is asking for UNLIMITED access to your tokens. If compromised, they can drain your balance completely.";
